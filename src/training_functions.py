...
def view_train_test_result(model, encoder_input_train, decoder_output_train, encoder_input_test, decoder_output_test):
    ...
    output_trainset = model(torch.from_numpy(encoder_input_train[1000:2280]).to(device).float())
    output_testset = model(torch.from_numpy(encoder_input_test[1000:2280]).to(device).float())
