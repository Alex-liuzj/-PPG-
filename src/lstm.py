...
def get_mse(model, encoder_input_train, decoder_output_train, encoder_input_test, decoder_output_test):
    ...
    output_trainset = model(torch.from_numpy(encoder_input_train[100:7000]).to(device).float())
    output_testset = model(torch.from_numpy(encoder_input_test[100:7000]).to(device).float())
...
def train_lstm(model, device, train_loader, num_epochs=40, lr=0.0003, weight_decay = 1e-6, encoder_input_test=None, decoder_output_test=None):
        ...
        for i, (inputs, labels) in enumerate(train_loader):
            ...
            if encoder_input_test is not None and decoder_output_test is not None:
                if (i + 1) % 100 == 0:
                    ...
                    print('train_mse: {:.4f}, test_mse: {:.4f}'.format(train_mse, test_mse))
